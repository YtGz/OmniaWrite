...
const storeState = () => {
  ...
  return {
     ...
     */
    updateCloudTimestamp: timestamp =>
      update(n => {
...
const storeProjects = () => {
  ...
  return {
    ...
    updateProjectTimestamp: id =>
      update(n => {
        n[n.findIndex(p => p.id == id)].lastOpen = (
          +new Date() / 1000
        ).toFixed();
        return n;
      }),
...
const storeCards = () => {
  ...
  return {
     ...
     */
    createCard: (project, title, content, showTooltip) =>
      update(n => {
        return n.concat([
          {
            id: getRandomNumber(),
...
export const intern = writable(
  JSON.parse(localStorage.getItem("intern") || "{}")
);
